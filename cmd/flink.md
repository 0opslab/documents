...
# 默认情况下，检查点是禁用的，需要手动启用检查点
# Flink 检查点需要两个前提：
# 	* 持久化数据源支持重放（Replay）数据，如 Kafka 和 HDFS
# 	* 用于持久化状态的存储，如 HDFS 和 RocksDB；
# 保存点通过 Flink 检查点机制保存了任务运行过程中状态的镜像。通常用于停止并恢复、分发和任务更新.如果不指定保存点目录，默认为 `state.savepoints.dir` 配置，可以通过编辑 conf/flink-conf.yml 文件修改：state.savepoints.dir: hdfs://RM/user/flink/savepoints 
# 触发保存点
